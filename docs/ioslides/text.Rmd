...
author: "Daniel Schalk<span style='padding-left:510px;'>15th and 16th March 2017</span>"
output: 
...
important to constitute a linear structure of the word vector space.
...
a sparce matrix. We don't want to drop the sparsity due to convenient
...
-->
Aggarawal et. al. have shown, that in high dimensions the ratio of the 
...
$\Rightarrow$ Points are concentrated on the surface of a hypersphere 
...
To evaluate trained word vectors, Mikolov et al. provide a word
...
- Often crawled from the web. This is mostly followed by a lot of 
  preprocessing (regular expressions, filtering stopwords etc.).
  
- Does different corpora imply a different quality of the word vectors?
  
- How big should the corbus and the vocabular be to get good word vectors?

<!-- - Stopwords from http://snowballstem.org -->

## Data: Common Sources
  
- **Wikipedia Dump + Gigaword 5:** Wikpedia gives access to all articles 
  collected within one XML file (unzipped about 63 GB). Pennington et. al. 
...
- **Twitter:** Pennington et. al. crawled 2 billion tweets. <br><br>
...
- ... the `R` package `text2vec`.

- ... the full wikipedia dump with first 7 million words out of 21 million
  (due to memory consumptions).
  
- ... the same settings as Pennington et. al.:
    - $\alpha = 0.75$
    - $x_\mathrm{max} = 100$
...
- Accuracy of word analogy task depends not only on the dimension, but 
  also on the corpus
