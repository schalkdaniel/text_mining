...
at this stage but can be any function.
...
<li>
   ...
   bias term we can decompose into a term $b_i$ from $w_i$ and one term 
...
a sparce matrix. We don't want to drop that property due to very convenient
...
Selecting this function also introduces two new hyperparameters $\alpha$ and 
...
for all tuning methods we need to evaluate the model. Since we want to model an
unsupervised task evaluating the model isn't straightforward.
...
## Validation: Questions Words File
<!--
