...
- Of course we want to have useful word vectors $w_i$ (whatever that means).
...
In general we could use every metric (norm) to evaluate the model. But the 
...
- Pennington et. al. have tuned the model and came to good values (just empirical
  without a proof):
    - $\alpha = 0.75$
...
## Outlook: Text Clustering 

- One way to continue using Glove is to use it as word embedding for 
  further applications.
  
- An example is text clustering:
    1. 
...
- Using server or pcs with huge computing power!
