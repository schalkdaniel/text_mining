...
# Helper function:
evaluateGloveOwn = function (word_vectors, a, b = NA, c = NA, norm, 
  print.words = 5L, print.sentence = TRUE)
{
  if (missing(norm)) {
    norm = function (x) 
    {
  ...
  if (! a %in% rownames(word_vectors)) {
    stop ("Word ", a, " doesn't occur as a word vector!")
  }
  if ( (! b %in% rownames(word_vectors)) && (! is.na(b)) ) {
    stop ("Word ", b, " doesn't occur as a word vector!")
  }
  if ( (! c %in% rownames(word_vectors)) && (! is.na(c)) ) {
